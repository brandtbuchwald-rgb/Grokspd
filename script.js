...
document.getElementById('furyBtn')?.addEventListener('click', ()=>{
  ...
  els.fury.checked = !on;
  btn.textContent = i18n[currentLang][els.fury.checked ? 'on' : 'off'];
...
function recalc(){
  ...
  if (s.cls === 'Berserker'){ furyRow?.classList.remove('hidden'); }
    ...
    if(b){ b.setAttribute('data-on','0'); b.classList.remove('is-active'); b.textContent=i18n[currentLang]['off']; }
  }
  ...
  els.progressText.textContent = `${i18n[currentLang].progress}${(progress*100).toFixed(1)}%${i18n[currentLang].ofRequiredCovered}`;
  ...
  if(plans.length){
    ...
    els.bestWaste.textContent = `${i18n[currentLang].wasteOverlap}${(lastBest.waste*100).toFixed(2)}%`;
...
function lineOf(r){
  return `${r.set}: ${r.pieces} piece${r.pieces===1?'':'s'} (${(r.equipPct*100).toFixed(2)}%)  |  Rune ${r.rune}%  |  Pet ${r.pet}  |  Quicken Lv.${r.quickLevel}`;
}
